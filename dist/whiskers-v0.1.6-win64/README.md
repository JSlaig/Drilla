...
Non-legacy tunnels use the standard `-J` jump syntax (clean, one password prompt
per host). For **legacy** tunnels with jump hosts, OpenSSH's `-J` negotiates the
middle hops from nested `ssh -W` processes that ignore command-line `-o` options,
so those hops couldn't use the legacy algorithms. For legacy tunnels whiskers
therefore builds the jump chain itself as an explicit nested `ProxyCommand`, so
every hop is negotiated by a local ssh carrying the tunnel's options — without
touching `~/.ssh/config`.
